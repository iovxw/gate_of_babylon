...
module a() {
    difference() {
        union() {
            translate([0, 0, 0]) a_bottom_bolt();
            
            knurl(k_cyl_hg=3, k_cyl_od=11, s_smooth=90, e_smooth=0.5,
                 knurl_wd=4, knurl_hg=3);
            
            translate([0, 0, 3]) a_top_bolt();
            
            /*intersection() {
                translate([0, 0, 0]) cylinder(h=10, d=10, $fn=6);
                translate([0, 0, -1]) sphere(d=15);
            }*/
        }
        cylinder(d=4.2, h=100, center=true);
        cylinder(d2=4.2, d1=4.4, h=3);
        mirror([0, 0, 1]) cylinder(d=4.4, h=100);
        //translate([0, 0, 1]) cylinder(d=4+0.5, h=100);
    }
}

w=0.3;
module a_top_bolt() difference () {
    imperial() tsmthread(DMAJ=0.35+$OD_COMP, PITCH=0.07, L=0.07*5, PR=THREAD_BUTT);
    translate([-w/2, 0, 0]) cube([w, 99, 99]);
    //rotate([0, 0, 360/3]) translate([-w/2, 0, 0]) cube([w, 99, 99]);
    //rotate([0, 0, 360/3*2]) translate([-w/2, 0, 0]) cube([w, 99, 99]);
}

module a_bottom_bolt() {
    /* 1/8" BSPT. */
    // from http://www.jyeu.com.tw/upload/files/1328751451.pdf
    // https://amesweb.info/Screws/bspp-thread-chart-calculator.aspx
    imperial()
        rotate([180, 0, 0])
        thread_bspt(DMAJ=0.3829, PITCH=0.0357, L=0.0357*7);
}

module b() {
    h=0.07*5*25.4+(0.5+0.8);
    difference() {
        knurl(k_cyl_hg=h, k_cyl_od=11, s_smooth=90, e_smooth=0.5,
            knurl_wd=4, knurl_hg=3);
        
        translate([0, 0, 0.5])
            imperial()
            tsmthread(DMAJ=0.35/*+$ID_COMP*/, PITCH=0.07, L=0.07*5, PR=THREAD_BUTT);
        
        cylinder(d=0.35*25.4/*+$ID_COMP*/, h=0.5);
...
THREAD_BUTT=prof_butt(30,0.2-0.000001); // Fix the error
...
module thread_bspt(DMAJ=10, L=10, PITCH=2.5, TAPER=1+(47/55), STUB=0) {
    PR=prof_bspt(TAPER);
//    echo(PR);
//    echo(add_npt(TAPER));
//    echo(TAPER);
...
function prof_bspt(TAPER=30, H1=0.8/2, H2=0.8/2) =
    let(M=tan(TAPER), PR2=delta(prof(55,H1,H2))) 
    integ([
            [0,0],
            PR2[0],
            PR2[1]+M*normy(PR2[1])*(PR2[1][0]+PR2[0][0]),
