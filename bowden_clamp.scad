...
module a() {
    difference() {
        union() {
            translate([0, 0, -10]) a_bottom_bolt();
        ...
        cylinder(d=2+0.5, h=100, center=true);
        translate([0, 0, 1]) cylinder(d=4+0.5, h=100);
    }
}

module a_top_bolt() {
    specs = thread_specs("M12-ext");
    P = specs[0]; Rrot = specs[1]; Dsupport = specs[2];
    section_profile = specs[3];
    difference() {
        translate([0, 0, 5+P/2]) bolt("M12", turns=2);
        translate([0, 0, 5+((P*2+P)-3.2)]) m4_nut();
    }
}
...
module a_bottom_bolt() {
   ...
   translate([0, 0, P/2]) bolt("M6", turns=10/P-1);
   translate([0, 0, -5]) cylinder(d=Dsupport, h=5);
}

module b() {
    specs = thread_specs("M12-int");
    P = specs[0]; Rrot = specs[1]; Dsupport = specs[2];
    section_profile = specs[3];
    intersection() {
        difference() {
        ...
        translate([0, 0, -2*P-P]) cylinder(d1=30, d2=0, h=12);
    }
    
    translate([0, 0, -2*P-P/2]) nut("M12", turns=2);
}
